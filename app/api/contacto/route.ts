  ...
  try {
    ...
    if (toEmail && process.env.RESEND_API_KEY) {
      ...
      const { error } = await resend.emails.send({
        from: 'Quinta Escala Contact <onboarding@resend.dev>',
        to: [toEmail],
        replyTo: email,
        subject: `[Quinta Escala] Nuevo contacto de ${name}`,
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p style="font-size:12px; color:#999;">Enviado desde el formulario de contacto de Quinta Escala</p>
          </div>
        `,
      });
